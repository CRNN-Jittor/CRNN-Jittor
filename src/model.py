...
class CRNN(nn.Module):
    ...
    def _cnn_backbone(self, img_channel, img_height, img_width, leaky_relu):
        ...
        conv_relu(0)
        cnn.add_module('pooling0', nn.Pool(2, stride=2, op='maximum'))
        # (64, img_height // 2, img_width // 2)

        conv_relu(1)
        cnn.add_module('pooling1', nn.Pool(2, stride=2, op='maximum'))
        # (128, img_height // 4, img_width // 4)

        conv_relu(2)
        conv_relu(3)
        cnn.add_module('pooling2', nn.Pool((2, 1), op='maximum'))
        # (256, img_height // 8, img_width // 4)

        conv_relu(4, batch_norm=True)
        conv_relu(5, batch_norm=True)
        cnn.add_module('pooling3', nn.Pool((2, 1), op='maximum'))
